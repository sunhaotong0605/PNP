...
class DnaBert2Embedding(BaseEmbedding):
    def __init__(self, pretrained_model_name_or_path: str, device: str, ignore_mismatched_sizes=True):
        super(DnaBert2Embedding, self).__init__()
        ...
        self.base_embed = base_model.bert
...
class DnaBert2ForClassifier(BaseForClassifier):
    def __init__(
            self,
        ...
        embedding = DnaBert2Embedding(pretrained_model_name_or_path, device=device)
        super().__init__(
            embedding=embedding,
            device=device,
...
class DnaBert2LoraForClassifier(BaseLoraForClassifier):
    def __init__(
            self,
            pretrained_model_name_or_path: str,
            num_classes: int = 2,
        ...
        peft_config.target_modules = ['Wqkv','gated_layers','wo','classifier']
        super().__init__(
            embedding=embedding,
...
class DnaBert2ForMultiTaskSequence(BaseForMultiTaskSequence):
    def __init__(
            self,
            pretrained_model_name_or_path: str,
            multi_task: dict,
